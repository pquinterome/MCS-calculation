...
model1.fit(x= X_train1, y =y_train, validation_data= (X_test1, y_test), callbacks=[early_stop, reduce_lr], epochs=2, verbose=0)
model2.fit(x= X_train1, y =y_train2, validation_data= (X_test1, y_test2), callbacks=[] ,epochs=2, verbose=0)
...
plt.subplot(211)
plt.title('Loss [rmse]')
plt.plot(metrics2[['loss', 'val_loss']], label=['train', 'loss'])
plt.legend()
plt.subplot(212)

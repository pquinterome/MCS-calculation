...
predictions = np.round(pred)
print('MU_Model Classification Report')
print(classification_report(y_test, predictions))
accuracy = accuracy_score(y_test, predictions)
...
predictions = np.round(pred)
print('Portal_Model Classification Report')
print(classification_report(y_test, predictions))
accuracy = accuracy_score(y_test, predictions)
fpr1, tpr1, thresholds = roc_curve(y_test, pred)
roc_auc = auc(fpr1, tpr1)
...
plt.xlim(-0.05, 1.05)
plt.ylim(-0.05, 1.05)
plt.savefig('output/AUC_portal.png', bbox_inches='tight')
##############################################
...
#-  compile model    
    ...
    model2.compile(loss="binary_crossentropy", optimizer= 'adam', metrics=['accuracy', roc])
    model2.fit(x=X[train], y= y[train], validation_data=(X[test], y[test]) ,epochs=2, batch_size=5, verbose=0, callbacks=[early_stop, reduce_lr])

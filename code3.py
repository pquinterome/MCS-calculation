...
for model in models:
    ...
    model.fit(x=[X_train1, X_train3], y= y_train, validation_data= ([X_test1, X_test3], y_test), epochs=200 ,verbose=0, callbacks=[early_stop])
    #model.fit_generator(train_generator, validation_data= test_generator, callbacks=[early_stop], epochs=200, verbose=0)
  
    y_pred_keras = model.predict((X_test1, X_test3)).ravel() 
...
pred = model.predict((X_test1, X_test3))
...
model4.fit(x=[X_train1, X_train3], y= y_train2, validation_data= ([X_test1, X_test3], y_test2), callbacks=[early_stop, reduce_lr], epochs=200, verbose=0)
pred2 = model4.predict((X_test1, X_test3))

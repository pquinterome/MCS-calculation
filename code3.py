...
roc = tf.keras.metrics.AUC(name='roc')
model1.compile(loss="binary_crossentropy", optimizer= 'adam', metrics=['accuracy', roc])
model1.fit(x=X_train3, y= y_train, validation_data=(X_test3, y_test) ,epochs=200, verbose=0, callbacks=[early_stop, reduce_lr])
pred = model1.predict(X_test3)
...
model1.fit(x= X_train3, y =y_train2, validation_data= (X_test3, y_test2), callbacks=[reduce_lr], epochs=200, verbose=0)
pred2 = model1.predict(X_test3)
...
i=1
metrics2 = pd.DataFrame(model1.history.history)
plt.figure(6)
plt.title('Loss')

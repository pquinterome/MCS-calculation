...
for model in models:
    ...
    y_pred_keras2 = model2.predict(X_test3).ravel() 
...
ax1.plot(mean_fpr1, mean_tpr, color='blue', linestyle='--',label=r'M_3(a) ROC_AUC = %0.2f $\pm$ %0.2f' % (mean_auc, std_auc), lw=2, alpha=.7)
std_tpr = np.std(tprs1, axis=0)
ax1.plot(mean_fpr2, mean_tpr2, color='green' , linestyle='-.',label=r'M_3(b) ROC_AUC = %0.2f $\pm$ %0.2f' % (mean_auc2, std_auc2), lw=2, alpha=.7)
std_tpr2 = np.std(tprs2, axis=0)
ax1.plot(mean_fpr3, mean_tpr3, color='#F97306', linestyle=':',label=r'M_3(c) ROC_AUC = %0.2f $\pm$ %0.2f' % (mean_auc3, std_auc3), lw=2, alpha=.7)
...
ax1.fill_between(mean_fpr1, tprs_lower, tprs_upper, color='blue', alpha=.2, label=r'$\pm$ 1 std. dev. M_3(a)')
ax1.fill_between(mean_fpr2, tprs_lower2, tprs_upper2, color='green', alpha=.2, label=r'$\pm$ 1 std. dev. M_3(b)')
ax1.fill_between(mean_fpr3, tprs_lower3, tprs_upper3, color='orange', alpha=.2, label=r'$\pm$ 1 std. dev. M_3(c)')
...
plt.savefig('output/M_3_outputs.png', bbox_inches='tight')



metrics1 = pd.DataFrame(model1.history.history)
pred1 = model1.predict(X_test3) ## change X3
...
pred2 = model2.predict(X_test3) ### change X3
